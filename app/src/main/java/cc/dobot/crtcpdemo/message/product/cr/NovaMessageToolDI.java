...
public class NovaMessageToolDI extends BaseMessage {

    private int index=1;
    @Override
    public void constructSendData() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            this.messageContent=("ToolDI("+index+")").getBytes(StandardCharsets.US_ASCII);
        }else
            this.messageContent=("ToolDI("+index+")").getBytes( Charset.forName("US-ASCII"));
        this.messageStrContent=("ToolDI("+index+")");
    }
