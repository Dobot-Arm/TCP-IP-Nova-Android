...
public class NovaMessageDI extends BaseMessage {

    private int index=1;
    @Override
    public void constructSendData() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            this.messageContent=("DI("+index+")").getBytes(StandardCharsets.US_ASCII);
        }else
            this.messageContent=("DI("+index+")").getBytes( Charset.forName("US-ASCII"));
        this.messageStrContent=("DI("+index+")");
    }
