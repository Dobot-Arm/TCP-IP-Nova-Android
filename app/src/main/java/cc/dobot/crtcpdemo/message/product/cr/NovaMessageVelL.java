...
public class NovaMessageVelL extends BaseMessage {

    private int R=1;
    @Override
    public void constructSendData() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            this.messageContent=("VelL("+R+")").getBytes(StandardCharsets.US_ASCII);
        }else
            this.messageContent=("VelL("+R+")").getBytes( Charset.forName("US-ASCII"));
        this.messageStrContent=("VelL("+R+")");
    }
