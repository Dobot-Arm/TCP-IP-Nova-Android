...
public class NovaMessageModbusClose extends BaseMessage {

    private int index=1;
    @Override
    public void constructSendData() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            this.messageContent=("ModbusClose("+index+")").getBytes(StandardCharsets.US_ASCII);
        }else
            this.messageContent=("ModbusClose("+index+")").getBytes( Charset.forName("US-ASCII"));
        this.messageStrContent=("ModbusClose("+index+")");
    }
