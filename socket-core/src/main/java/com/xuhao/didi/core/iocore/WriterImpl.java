    ...
    public boolean write() throws RuntimeException {
        ...
        if (sendable != null) {
            try {
                ...
                Buffer writeBuf = ByteBuffer.allocate(packageSize);
                ((ByteBuffer) writeBuf).order(mOkOptions.getWriteByteOrder());
                int index = 0;
                while (remainingCount > 0) {
                    int realWriteLength = Math.min(packageSize, remainingCount);
                    writeBuf.clear();
                    writeBuf.rewind();
                    ((ByteBuffer) writeBuf).put(sendBytes, index, realWriteLength);
                    writeBuf.flip();
                    byte[] writeArr = new byte[realWriteLength];
                    ((ByteBuffer) writeBuf).get(writeArr);
